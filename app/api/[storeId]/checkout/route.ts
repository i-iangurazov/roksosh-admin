...
export async function POST(
  ...
  fetchedProducts.forEach((product) => {
      ...
      if (productDetails?.colorId && !product.colors.some((color) => color.id === productDetails.colorId)) {
          throw new Error(`Invalid color selection for product ${product.id}`);
      }

      if (productDetails?.sizeId && !product.sizes.some((size) => size.id === productDetails.sizeId)) {
          throw new Error(`Invalid size selection for product ${product.id}`);
      }
